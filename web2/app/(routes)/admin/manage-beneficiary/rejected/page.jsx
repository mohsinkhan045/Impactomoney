...
export default function RejectedBeneficiary() {
  ...
  // Memoize fetchBeneficiaries function
  const fetchBeneficiaries = useCallback(async (token) => {
    try {
      ...
      const rejectedBeneficiaries = data.data.beneficiaries.filter(
        (beneficiary) => beneficiary.status === "rejected"
      );
      setBeneficiaries(rejectedBeneficiaries || []);
    } catch (error) {
  ...
  const BeneficiaryRow = React.memo(({ beneficiary, index, getVoucherDetails }) => (
    <tr className="border-b border-gray-200 transition-transform duration-500  hover:border-2  hover:bg-gray-100">
      ...
      <td className="py-1 px-3 text-red-700 text-lg font-semibold">{beneficiary.status}</td>
    </tr>
  ));
  ...
  return (
    <div className="p-0 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-between">
        <h2 className="text-2xl font-bold mb-4">Rejected Beneficiaries</h2>
