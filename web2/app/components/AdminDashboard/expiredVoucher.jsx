...
const ExpiredVoucher = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/admin/getIssuedVouchers?isExpired=true", {
          method: "GET",
          headers: {
      ...
      <div className="p-0 md:p-2 mt-4 md:mt-10">
        <div className="flex flex-col gap-2 md:flex-row justify-between">
          <div className="flex-1">
            <h1 className="text-2xl font-bold mb-2">Expired Vouchers</h1>
            <p className="text-gray-600 mb-6">
              Expired Vouchers to the beneficiaries are below
            </p>
          </div>
          <div className="p-2">
            <button
